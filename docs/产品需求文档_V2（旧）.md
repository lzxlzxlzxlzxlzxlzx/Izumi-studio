...
|------|------------|--------------|
| 前端技术 | jQuery + 单页 HTML | React + TypeScript，组件化 |
| 界面风格 | 功能堆砌，配置面板密集 | 现代卡片式 UI，沉浸感强 |
| 对话模式 | 单一对话界面 | 创作 / 游玩 / 聊天三种分离模式 |
| 多模态 | 图片理解（有限）| 图片理解 + 图片生成（完整） |
| 部署方式 | 本地 Node.js | FastAPI 后端 + React 前端，浏览器访问 |
...
- **单一职责**：每个模块只负责一个核心功能领域，模块间通过接口通信
- **接口明确**：每个模块定义清晰的输入/输出类型，边界清晰
- **单向依赖**：上层模块依赖下层模块，避免循环依赖
- **可替换性**：同一接口可对应多种实现（如 LLM 供应商切换、图片服务切换）
...
│                    UI 层（React）                             │
│  ┌──────────┐  ┌──────────┐  ┌──────────────────────────┐   │
│  │ 角色卡选择 │  │ 对话界面  │  │ 配置面板（抽屉/弹窗）      │   │
│  └─────┬────┘  └─────┬────┘  └──────────┬───────────────┘   │
│        │             │                  │                   │
│  ┌─────▼─────────────▼──────────────────▼───────────────┐   │
│  │                State Layer (Zustand)                   │   │
│  │   sessionStore │ cardStore │ chatStore │ configStore   │   │
│  └────────────────────────┬──────────────────────────────┘   │
└───────────────────────────┼──────────────────────────────────┘
                            │ REST API
┌───────────────────────────┼──────────────────────────────────┐
...
│  │                  上下文组装管线                          │   │
│  │  ┌──────────┐  ┌───────────┐  ┌──────────────────┐    │   │
│  │  │WorldBook │→│PromptMgr  │→│ContextAssembler   │    │   │
│  │  │Engine    │  │(排序)     │  │(核心组装+Token预算)│    │   │
│  │  └──────────┘  └───────────┘  └───────┬──────────┘    │   │
│  │  ┌──────────┐  ┌───────────┐          │               │   │
│  │  │Memory    │→│Template   │──────────┘               │   │
│  │  │System    │  │Renderer   │                          │   │
...
│  ┌────────────────────────▼──────────────────────────────┐   │
│  │                    LLMRouter                            │   │
│  │      Claude │ Qwen │ DeepSeek │ Kimi │ Azure OpenAI    │   │
│  └─────────────────────────────────────────────────────────┘   │
└────────────────────────────────────────────────────────────────┘
```
...
#### 13.3.1 角色卡管理模块（CharacterCardManager）

**职责：** 角色卡的 CRUD、导入导出、版本管理。

| 输入 | 类型 | 说明 |
|------|------|------|
| `card_id` | `string` | 角色卡 ID |
| `import_file` | `File` | 导入文件（JSON / PNG） |
| `card_data` | `CharacterCard` | 角色卡完整数据 |

| 输出 | 类型 | 说明 |
|------|------|------|
| `character` | `CharacterCard` | 角色卡对象（含格式化后的各字段） |
| `validation_result` | `ValidationResult` | 导入验证结果 |

**依赖：** 无

---

#### 13.3.2 世界书引擎（WorldBookEngine）

**职责：** 管理世界书条目，执行关键词扫描和触发逻辑，返回需注入的条目列表。

**输入：**

| 参数 | 类型 | 说明 |
|------|------|------|
| `chat_history` | `Message[]` | 最近 N 轮聊天历史 |
| `worldbook_config` | `WorldBookConfig` | 世界书元数据（scan_depth, token_budget 等） |
| `entries` | `WorldEntry[]` | 所有可用条目 |
| `timed_effects` | `TimedEffects` | 跨轮次持久化的时序效果（sticky/cooldown/delay） |
| `persona` | `Persona` | 当前用户人设（扩展扫描） |
| `character` | `CharacterCard` | 当前角色卡（扩展扫描） |

**输出：**

| 返回值 | 类型 | 说明 |
|--------|------|------|
| `before_char_entries` | `WorldEntry[]` | position=before_char 的条目 |
| `after_char_entries` | `WorldEntry[]` | position=after_char 的条目 |
| `at_depth_entries` | `AtDepthEntry[]` | position=at_depth 的条目（含 depth） |
| `example_entries` | `WorldEntry[]` | position=examples 的条目 |
| `an_entries` | `WorldEntry[]` | position=an_top/an_bottom 的条目 |
| `updated_effects` | `TimedEffects` | 更新后的时序效果状态 |

**依赖：** 无

---

#### 13.3.3 三层记忆系统（MemorySystem）

**职责：** 短期/中期/长期记忆的生成、存储和检索。
...
| `card_id` | `string` | 角色卡 ID |
| `chat_history` | `Message[]` | 完整聊天历史 |
| `trigger_type` | `'round' | 'token' | 'manual'` | 摘要触发类型 |

**输出：**

| 返回值 | 类型 | 说明 |
|--------|------|------|
| `short_term` | `Message[]` | 最近 N 轮原始消息 |
| `mid_term` | `Summary[]` | 阶段摘要列表（最多 3 段） |
| `long_term` | `Fact[]` | 关键事实列表 |
| `new_summary` | `Summary` | 最新生成的摘要（如有） |

**依赖：** LLMRouter（摘要生成调用）

---

#### 13.3.4 上下文模板渲染器（ContextTemplateRenderer）

**职责：** 将 Handlebars 上下文模板渲染为最终 system prompt 字符串。

**输入：**

| 参数 | 类型 | 说明 |
|------|------|------|
| `template` | `string` | 上下文模板（story_string） |
| `vars` | `TemplateVars` | 渲染变量（description, personality, scenario 等） |

**输出：**

| 返回值 | 类型 | 说明 |
|--------|------|------|
| `rendered_prompt` | `string` | 渲染后的 system prompt |

**依赖：** 无

---

#### 13.3.5 指令格式化器（InstructFormatter）

**职责：** 根据指令模板配置，将每条消息格式化为目标模型所需的格式。
...
| `template` | `InstructTemplate` | 指令模板配置 |
| `name1` | `string` | 用户名称 |
| `name2` | `string` | 角色名称 |
...
| 返回值 | 类型 | 说明 |
|--------|------|------|
| `formatted_text` | `string` | 格式化后的消息文本 |

**依赖：** 无

---

#### 13.3.6 提示顺序管理器（PromptManager）

**职责：** 管理所有提示片段的顺序、启用状态和注入位置。

**输入：**

| 参数 | 类型 | 说明 |
|------|------|------|
| `character_id` | `string` | 当前角色卡 ID |
| `global_order` | `PromptOrder[]` | 全局默认顺序 |
| `generation_type` | `'normal' | 'continue' | 'impersonate'` | 生成类型 |

**输出：**

| 返回值 | 类型 | 说明 |
|--------|------|------|
| `prompt_collection` | `Prompt[]` | 排序后的活跃提示列表 |

**依赖：** 无

---

#### 13.3.7 上下文组装器（ContextAssembler）

**职责：** 核心模块。接收所有输入并组装为最终的 messages 数组，管理 Token 预算。

**输入：**

| 参数 | 类型 | 说明 |
|------|------|------|
| `character` | `CharacterCard` | 角色卡数据 |
| `worldbook_output` | `WorldBookOutput` | 世界书引擎输出的各位置条目 |
| `memory` | `MemoryOutput` | 三层记忆内容 |
| `persona` | `Persona` | 用户人设 |
| `authors_note` | `AuthorsNote` | Author's Note 配置 |
| `chat_history` | `Message[]` | 聊天历史 |
| `prompt_collection` | `Prompt[]` | PromptManager 输出的顺序 |
| `instruct_template` | `InstructTemplate` | 指令模板配置 |
| `context_template` | `ContextTemplate` | 上下文模板配置 |
| `token_budget` | `number` | token 预算上限（max_context - max_tokens） |
| `current_input` | `string` | 当前用户输入 |
| `system_prompt_override` | `string | null` | 角色卡系统提示覆盖 |
| `jailbreak_override` | `string | null` | 角色卡后历史指令覆盖 |

**输出：**

| 返回值 | 类型 | 说明 |
|--------|------|------|
| `messages` | `ChatMessage[]` | 最终 messages 数组 |
| `token_counts` | `TokenCounts` | 各组件 token 统计 |
| `trimmed_components` | `string[]` | 被裁剪的组件列表 |

**组装顺序（最终消息数组）：**

```
SYSTEM PROMPT 区域:
  1. worldInfoBefore     ← 世界书(before_char)
  2. main                ← ContextTemplateRenderer 渲染的主提示
  3. worldInfoAfter      ← 世界书(after_char)
  4. charDescription     ← 角色描述
  5. charPersonality     ← 角色性格
  6. scenario            ← 场景设定
  7. personaDescription  ← 用户人设描述
  8. summary             ← 中期记忆摘要
  9. longTermMemory      ← 长期记忆
  10. authorsNote        ← Author's Note（position=before_scenario/after_scenario）
  11. postHistory        ← Post History Instructions
  12. extensions         ← 扩展提示

CHAT HISTORY 区域:
  13. dialogueExamples   ← 示例对话（few-shot）
  14. chatHistory        ← 实际聊天历史（含 at_depth 注入的世界书条目）

CONTROL 区域:
  15. controlPrompt      ← impersonate/continue 等控制提示
```

**Token 预算法则：**
1. System prompt 区域必须全部容纳
2. 为"新对话提示"预留预算
3. 示例对话 → 能容纳多少加多少
4. 聊天历史 → 从最新消息反向添加，预算用完即止
5. 裁剪顺序：中期摘要（保留最新）→ 世界书条目（按 priority）→ 示例对话 → 最旧聊天历史

**依赖：** TemplateRenderer, InstructFormatter, PromptManager

---

#### 13.3.8 消息操作引擎（MessageEngine）

**职责：** 管理消息的 Swipe、Checkpoint、Branch、Continue、编辑、撤回等操作。

**输入：**

| 参数 | 类型 | 说明 |
|------|------|------|
| `operation` | `'swipe' | 'checkpoint' | 'branch' | 'edit' | 'delete' | 'lock' | 'continue'` | 操作类型 |
| `params` | `OperationParams` | 操作参数（消息 ID、内容、索引等） |
...
| 返回值 | 类型 | 说明 |
|--------|------|------|
| `updated_session` | `Session` | 更新后的会话数据 |

**依赖：** 无

---

#### 13.3.9 LLM 路由层（LLMRouter）

**职责：** 统一管理所有 LLM 供应商的 API 调用，处理格式转换、重试逻辑和流式输出。

**输入：**

| 参数 | 类型 | 说明 |
|------|------|------|
| `messages` | `ChatMessage[]` | ContextAssembler 输出的 messages |
| `model_config` | `ModelConfig` | 模型配置（供应商、temperature、max_tokens 等） |
| `stream` | `boolean` | 是否流式输出 |
| `tools` | `ToolDefinition[]` | 工具定义列表（搜索等） |

**输出：**

| 返回值 | 类型 | 说明 |
|--------|------|------|
| `response_stream` | `AsyncGenerator<Chunk>` | 流式响应块序列 |
| `full_response` | `string` | 完整响应文本 |
...
| 模块 | 职责 | 核心输入 | 核心输出 |
|------|------|---------|---------|
| **AuthorsNoteManager** | AN 的配置管理和注入参数计算 | config, round_number | content, position, depth, interval |
| **PersonaManager** | 用户人设的 CRUD 和锁定管理 | persona_id | persona content + lock info |
| **QuickReplyEngine** | 快捷回复集的 CRUD、合并和排序 | character_id, global_sets | merged button list |
| **RegexEngine** | 正则脚本的匹配替换执行 | text, scripts[], stage | processed text |
| **SearchTool** | 联网搜索工具调用 | query string | search results |
| **ImageService** | 多服务图片生成 | prompt, config, references | image data |
| **UIStateEngine** | 状态栏/悬浮球的状态维护和更新 | STATE_UPDATE 指令 | updated state values |

### 13.4 核心数据流时序

```
用户发送消息:
  ① UI → POST /v1/chat（含当前输入 + session_id）
  ② 后端加载角色卡、会话、世界书数据
  ③ WorldBookEngine.scan(chat_history, config)       → 激活条目列表
  ④ MemorySystem.get(session_id)                      → 三层记忆
  ⑤ ContextTemplateRenderer.render(template, vars)    → system prompt
  ⑥ PromptManager.getOrder(card_id)                   → prompt_collection
  ⑦ ContextAssembler.assemble({所有输入})               → messages + token_counts
  ⑧ LLMRouter.send(messages, config)                  → 流式响应
  ⑨ 响应逐块 SSE 推送至前端
  ⑩ 响应完成后后端执行:
      - RegexEngine.process(response, 'model_output')  → 清理 STATE_UPDATE 等
      - MessageEngine.save(response, session)
      - WorldBookEngine.updateTimedEffects()
      - MemorySystem.checkAndTrigger(session_id)       → 按需生成摘要
```
...
MemorySystem ───────┤                          │
AuthorsNoteManager ─┤                          │
PersonaManager ─────┤──→ ContextAssembler ──→ LLMRouter
TemplateRenderer ───┤       │                   │
InstructFormatter ───┘       │                   │
PromptManager ───────────────┘                   │
                              │                  │
...
- **UI 组件库**：Ant Design（基础组件）+ 自定义组件
- **样式**：Tailwind CSS + CSS Variables（主题切换）
- **状态管理**：Zustand（轻量）+ 中间件（persist, devtools）
- **路由**：React Router v6
- **HTTP**：Axios
- **Markdown 渲染**：react-markdown + rehype-highlight（代码）+ KaTeX（数学）
- **流式输出**：EventSource / SSE + fetch ReadableStream
